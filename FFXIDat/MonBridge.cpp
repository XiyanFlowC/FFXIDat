...
void MonBridge::ToICsv(const std::wstring& path) const
{
    ...
    csv.NewCell(u8"Para1[0]");
    csv.NewCell(u8"Para1[1]");
    csv.NewCell(u8"Para1[2]");
    csv.NewCell(u8"Para1[3]");
    csv.NewCell(u8"Para1[4]");
    csv.NewCell(u8"Has_Icon");
    csv.NewLine();
    ...
    for (const auto& datum : data) {
        ...
        // Write para1 array (first 5 values)
        for (int i = 0; i < 5; ++i) {
            csv.NewCell(xybase::string::to_utf8(std::to_string(datum.originalEntry.para1[i])));
        }
